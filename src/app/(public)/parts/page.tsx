...
export default function PartsPage() {
  ...
  const supportService = getEligibleSupportServices().find(({ href }) => href === "/parts");
  if (!route?.status.startsWith("eligible") || contactActions.length === 0) notFound();
  return <InquiryPage type="parts" claims={claims} contactActions={contactActions} image={supportService?.image} />;
}
